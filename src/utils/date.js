...
export function fmtMoney(n){ const x=Number(n||0); return x.toLocaleString(undefined,{style:'currency',currency:'USD',maximumFractionDigits:2}) }
...
export function ymLabel(ym){ const [y,m]=ym.split('-').map(Number); const d=parseToDate(`${y}-${String(m).padStart(2,'0')}-01`); return format(d,'LLLL yyyy') }
